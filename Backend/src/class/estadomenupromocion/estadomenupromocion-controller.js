...
const EstadoMenuPromocionModelo = require("./estadomenupromocion-model"),
  EstadoMenuPromocionController = () => {},
  legend = "EstadoMenuPromocion",
  idtable = "idEstadoMenuPromocion",
  table = "estadomenupromocion";

EstadoMenuPromocionController.getAll = (req, res, next) => {
  EstadoMenuPromocionModelo.findAll({ raw: true }).then(projects => {
  ...
  if (req.body[idtable]) {
    EstadoMenuPromocionModelo.findOne({
      ...
      if (!project || project == 0) {
        ...
        if (check) {
          EstadoMenuPromocionModelo.update(req.body, {
            where: {
...
EstadoMenuPromocionController.delete = (req, res, next) => {
  let [idtabla] = req.params[idtabla];
  UsuarioEstadoMenuPromocionModelo.getOne([idtabla], (err, rows) => {
    if (err) {
      let locals = {
...
EstadoMenuPromocionController.error404 = (req, res, next) => {
  let error = new Error(),
    locals = {
      title: "Error 404",
