...
const UsuarioModelo = require("./usuario-model"),
  ...
  UsuarioController.create = (req, res) => {
    ...
    if (body[idtable]) {
      ...
      UsuarioModelo.findOne({ where: { [idtable]: body[idtable] }})
      .then(response => {
        if (!response || response == 0) {
...
//----------------------------------------
              if (actualizarEstado) {
                ...
                UsuarioEstadoModelo.update({fechaYHoraBajaUsuarioEstado: Date()}, {
                    where: { [idtable]: body[idtable], fechaYHoraBajaUsuarioEstado: null }
                })
                .then((resp) => {
                  if (!resp || resp == 0) {
...
//----------------------------------------
              if (actualizarRol){
                ...
                RolUsuarioModelo.update({fechaYHoraBajaRolUsuario: Date()}, {
                  where: { [idtable]: body[idtable], fechaYHoraBajaRolUsuario: null }
                })
                .then((resp) => {
                  if (!resp || resp == 0) {
