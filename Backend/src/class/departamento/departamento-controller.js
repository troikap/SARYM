...
DepartamentoController.getToAllAttributes = (req, res, next) => {
  let locals = {};
  DepartamentoModelo.findAll({
    where: {
      [Op.or]: [
        {idDepartamento: {[Op.substring]: req.params.anyAttribute}},
...
DepartamentoController.getToName = (req, res, next) => {
  let locals = {};
  DepartamentoModelo.findAll({
  ...
  if (body[idtable]) {
    DepartamentoModelo.findOne({
      ...
      if (!response || response == 0) {
        ...
        if ( 
          body.nombreDepartamento != response.dataValues.nombreDepartamento
          ) {actualizar = true}
        if (actualizar) {
          DepartamentoModelo.update(
            body, 
