...
const EstadoReservaModelo = require("./estadoreserva-model"),
  EstadoReservaController = () => {},
  legend = "EstadoReserva",
  idtable = "idEstadoReserva",
  table = "estadoreserva";

EstadoReservaController.getAll = (req, res, next) => {
  EstadoReservaModelo.findAll({ raw: true }).then(projects => {
  ...
  if (req.body[idtable]) {
    EstadoReservaModelo.findOne({
      ...
      if (!project || project == 0) {
        ...
        if (check) {
          EstadoReservaModelo.update(req.body, {
            where: {
...
EstadoReservaController.delete = (req, res, next) => {
  let [idtabla] = req.params[idtabla];
  UsuarioEstadoReservaModelo.getOne([idtabla], (err, rows) => {
    if (err) {
      let locals = {
